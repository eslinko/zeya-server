...
class Matches extends ActiveRecord
{
    ...
    static function checkGetUsersMatch($user_id_1, $user_id_2) {
        ...
        $match_actions = MatchAction::find()->where(['action_user_id' => $user_id_1, 'expression_user_id' => $user_id_2, 'action_result' => 1])->orderBy(['timestamp' => SORT_DESC])->all();
        if($match_actions === NULL) return false;
        $user_2_ce = CreativeExpressions::find()->where(['id' => $match_actions[0]->expression_id])->one();
        $match_actions = MatchAction::find()->where(['action_user_id' => $user_id_2, 'expression_user_id' => $user_id_1, 'action_result' => 1])->orderBy(['timestamp' => SORT_DESC])->all();
        if($match_actions === NULL) return false;
